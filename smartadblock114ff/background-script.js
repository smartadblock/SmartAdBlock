...
var extra_filters;
var dups=new Map();
var extra_filters_check=[];

var cssrule_s=new Map();
var allreqrules_s=[];
var reqdom_s=new Map();
var reqdomtarget_s=new Map();
var reqsub_s=new Map();
var csswhite_s=[];
var allwhite_s=[];
var reqwhite_s=[];
var sublongest_s=[];
...
var cookierules_s={};
cookierules_s["all"]=[];
var dangerforbrules_s=[];
var targetwhitelist_s=[];
var cssdelay_s={};
var reqforce_s={};
var reqforceinclude_s=[];
var nomulti_s={};
var topazf_s={};
var topazs_s={};
var topazdelay_s={};
...
function loadextra(force=true){
	chrome.storage.local.get("rules", function(result) {
		...
		loadextrafilters(force);
	});
}

function loadextrafilters(force=true){
...
function load_suffix(force=true){
	suf=[];
	var xhr = new XMLHttpRequest();
...
function check_for_storage(){
	chrome.storage.local.get("stored", function(result) {
		if(typeof result != 'undefined'){
			if(result["stored"]=="in"){
				console.log("Restoring Filters From Storage")
				restore_rules();
				loadextra(false);
			}
			else{
...
function getparenturl(detail){
	if(firefox){
		if(typeof(detail["originUrl"])!="undefined"){return detail["originUrl"];}
		if(typeof(detail["documentUrl"])!="undefined"){return detail["documentUrl"];}
		return "";
	}
	else{
		if(typeof(detail["initiator"])!="undefined"){return detail["initiator"];}
		if(typeof(detail["url"])!="undefined"){return detail["url"];}
		return "";
	}
}
...
function makecookierule(txt){
	...
	var arr=txt.split("##");
	if(arr[0]==""){
		cookierules_s["all"].push(arr[1]);
	}
	else{
		var arrc=arr[0].split(",");
		var ne = arrc.length;
		for (var i = 0; i < ne; i++) {
			if(!(arrc[i] in cookierules_s)){cookierules_s[arrc[i]]=[];}
				cookierules_s[arrc[i]].push(arr[1]);
		}
	}
}
...
function treatreqexception(txt,inc=false){
	txt=txt.replace("!#ri#","");
	txt=txt.replace("!#r#","");
	if(txt.includes("||")){
		var splitarr=txt.split("||");
		reqwhite_s.push(splitarr[0]);
		if(inc){reqforceinclude_s.push(splitarr[0]);}
		var splitroot=splitarr[0];
		var tail=splitarr[1];
		var tailarr=tail.split(",");
		var ne = tailarr.length;
		reqforce_s[splitroot]=[];
		for (var i = 0; i < ne; i++) {
			if(tailarr[i].length>0){
				reqforce_s[splitroot].push(tailarr[i]);
			}
		}
	}
	else{
...
function treattarif(txt){
	if(txt.includes("##")){
		...
		var webs=tarifarr[0];
		var rull=tarifarr[1];
		if(!(webs in tarif_s)){tarif_s[webs]="";}
		tarif_s[webs]+=" "+rull+sepdisp;
	}
}
...
function treatcss(origtxt,i,key="sab***all"){
	...
	if(txt.startsWith("!~!")){veryquick_s.push(txt.replace("!~!",""));return "css";}
	if(txt.startsWith("!D!")){dangerrules_s.push(txt.replace("!D!",""));return "css";}
	if(txt.startsWith("!C!")){makecookierule(txt);return "css";}
	if(txt.startsWith("!o->!")){popupsourcewhite_s.push(txt.replace("!o->!",""));return "css";}
	if(txt.startsWith("!->o!")){targetwhitelist_s.push(txt.replace("!->o!",""));return "css";}
	if(txt.startsWith("!flag!")){checkflag(txt);return "css";}
	if(txt.startsWith("!danger!")){dangerforbrules_s.push(txt.replace("!danger!",""));return "css";}
	if(txt.startsWith("!nomulti!")){nomulti_s[txt.replace("!nomulti!","")]="";return "css";}
	if(txt.startsWith("!*")){popuprisk_s.push(txt.replace("!*",""));return "css";}
	if(txt.startsWith("!js!")){jsrule_s.push(txt.replace("!js!",""));return "css";}
	if(txt.startsWith("!s!")){skipcss_s=skipcss_s.concat(txt.replace("!s!","").split(","));return "css";}
	if(txt.startsWith("!##")){allwhite_s.push(txt.replace("!##",""));return "css";}
	...
	if(txt.startsWith("!#ri#")){treatreqexception(txt.replace("*tr*",trackerlist),true);return "css";}
	if(txt.startsWith("!#t#")){txt=txt.replace("!#t#","");arr=txt.split("|");cssdelay_s[arr[0]]=arr[1];return "css";}
	if(txt.startsWith("!#")){csswhite_s.push(txt.replace("!#",""));return "css";}
	if(txt.startsWith("!") || txt.startsWith("[")){return "css";}
	if(txt.includes("#")){
		if(txt.includes("#@")){
			tarr=txt.split("#@#");
			if(tarr[0]==""){
				cssrulewhite_s["all"].push(tarr[1]);
				return "css";
			}
			else{
				arr=tarr[0].split(",");
				...
				for (var i = 0; i < ne; i++) {
					if(!(arr[i] in ignorelist)){
						if(!(arr[i] in cssrulewhite_s)){cssrulewhite_s[arr[i]]=[];}
						cssrulewhite_s[arr[i]].push(tarr[1]);
					}
				}
			}
			return "css";
			...
			if(txt.includes("##")){
				...
				if(tarr[0]==""){
					...
					if(nosli.includes("'")){
						sab_all_v_s.push(nosli);
					}
					else{
						if(key=="sab***iframe"){sab_iframe_s.push(nosli);}else{sab_all_s.push(nosli);}
					}
				}
				else{
					arr=tarr[0].split(",");
					...
					for (var i = 0; i < ne; i++) {
						if(!(arr[i] in ignorelist)){
							if(!(cssrule_s.has(arr[i]))){cssrule_s.set(arr[i],"");}
							var nosli=tarr[1].split('').join('');
							cssrule_s.set(arr[i],cssrule_s.get(arr[i])+nosli+sepdisp);
						}
					}
				}
				return "css";
...
function make_req_rules(easy_arr){
	...
	for (var i = 0; i < ne; i++) {
		...
		allreqrules_s.push(origtxt);
		var index=allreqrules_s.length-1;
		if(a!==false){
			m = a["domainw"].length;
			if(m>0){
				for (var j = 0; j < m; j++) {
					if(!(a["domainw"][j] in ignorelist)){
						if(!reqdom_s.has(a["domainw"][j])){reqdom_s.set(a["domainw"][j],"")}
						bef=reqdom_s.get(a["domainw"][j]);
						xindex=index;
						if(a["domtarget"]){xindex=-1*index;}
						if(bef==""){aft=xindex}else{aft=bef+','+xindex}
						reqdom_s.set(a["domainw"][j],aft);
						d++;
					}
				}
			}
			if(m==0 && a["domtarget"]){
				if(!(a["root"] in ignorelist)){
					if(!reqdomtarget_s.has(a["root"])){reqdomtarget_s.set(a["root"],"")}
						bef=reqdomtarget_s.get(a["root"]);
						if(bef==""){aft=index}else{aft=bef+','+index}
						reqdomtarget_s.set(a["root"],aft);
						dt++;
				}
			}
			if(m==0 && !a["domtarget"]){
				...
				cin=false;
				if(best.toLowerCase().includes("ad")){ad_c=ad_c+1;cin=true}
				if(best.toLowerCase().includes("ads")){ads_c=ads_c+1;}
				if(best.includes("x") && cin==false){promo_c=promo_c+1;cin=true}
				if(best.includes("_") && cin==false){spons_c=spons_c+1;}
				sublongest_s.push(best);
				subrules_s.push(index);
				s++;
			}
		}
	}
...
function extracttext(arr){
	...
	if(arr.length==0){
		...
		var nnp=popuprisk_s.length;
		var popupriskjson={};
		for (var i = 0; i < nnp; i++) {
			popupriskjson[popuprisk_s[i]]="";
		}
		...
		var nnp=jsrule_s.length;
		var jsrulejson={};
		for (var i = 0; i < nnp; i++) {
			jsrulejson[jsrule_s[i]]="";
		}
	...
	xhr.onreadystatechange = function()
	{
		...
		{
			if(txt.includes("https://easylist.to/easylist/easylist.txt")){
			dlerror=true;
			}
		}
...
function initfilters(err=""){
	...
	if(typeof result != 'undefined'){
		...
		extra_filters_check=res;
	}catch{}
	}else{}
	if(extra_filters_check.length==0){
		extra_filters_check=["sab","fc","el"];
	}
	var inp=make_filterlist(err);
...
function make_filterlist(err=""){
	...
	mapper["sab"]=['https://www.smartadblock.co.uk/filters/sabfilter.txt'];
	mapper["sab"].push('https://www.smartadblock.co.uk/filters/sabfilter_short.txt');
	mapper["fc"]=[fbp+'cookie_general_hide.txt'];
	mapper["fc"].push(fbp+'cookie_whitelist_general_hide.txt');
	mapper["fc"].push(fbp+'cookie_specific_hide.txt');
	//mapper["fc"].push(fbp+"cookie_international_specific_hide.txt");
	...
	var ee="https://easylist.to/easylist/";
	var ed="https://easylist-downloads.adblockplus.org/";
	mapper["el"]=["https://easylist.to/easylist/easylist.txt"];
	if(err=="second"){
	mapper["el"]=[elp+'adservers.txt'];
	mapper["el"].push(elp+'general_block.txt');
	mapper["el"].push(elp+'general_block_dimensions.txt');
	mapper["el"].push(elp+'general_hide.txt');
	mapper["el"].push(elp+'specific_block.txt');
	mapper["el"].push(elp+'specific_hide.txt');
	mapper["el"].push(elp+'thirdparty.txt');
	mapper["el"].push(elp+'whitelist.txt');
	mapper["el"].push(elp+'whitelist_dimensions.txt');
	mapper["el"].push(elp+'whitelist_general_hide.txt');
	}
	mapper["ep"]=["https://easylist.to/easylist/easyprivacy.txt"];
	mapper["leg"]=["https://easylist.to/easylistgermany/easylistgermany.txt"];
	mapper["lei"]=[ed+"easylistitaly.txt"];
	mapper["led"]=[ed+"easylistdutch.txt"];
	mapper["lef"]=[ed+"liste_fr.txt"];
	mapper["lec"]=[ed+"easylistchina.txt"];
	mapper["leb"]=["http://stanev.org/abp/adblock_bg.txt"];
	mapper["lein"]=["https://raw.githubusercontent.com/heradhis/indonesianadblockrules/master/subscriptions/abpindo.txt"];
	mapper["lea"]=[ed+"Liste_AR.txt"];
	mapper["les"]=["https://raw.githubusercontent.com/tomasko126/easylistczechandslovak/master/filters.txt"];
	mapper["lel"]=["https://notabug.org/latvian-list/adblock-latvian/raw/master/lists/latvian-list.txt"];
	mapper["leh"]=["https://raw.githubusercontent.com/easylist/EasyListHebrew/master/EasyListHebrew.txt"];
	mapper["leli"]=["http://margevicius.lt/easylistlithuania.txt"];
	mapper["ar"]=["https://easylist-downloads.adblockplus.org/antiadblockfilters.txt"];
	mapper["pl"]=["https://pgl.yoyo.org/as/serverlist.php?hostformat=adblockplus;showintro=0"];
	mapper["lar"]=["https://easylist-downloads.adblockplus.org/ruadlist+easylist.txt"];
	mapper["fa"]=[ee+"fanboy-annoyance.txt"];
	mapper["fs"]=[ee+"fanboy-social.txt"];
	mapper["ws"]=["https://raw.githubusercontent.com/smartadblock/SmartAdBlock/master/w3techs_200.txt"];
	mapper["oh"]=["https://raw.githubusercontent.com/smartadblock/SmartAdBlock/master/1hosts.txt"];
	mapper["dm"]=["https://raw.githubusercontent.com/smartadblock/SmartAdBlock/master/Disconnect_malvertising.txt"];
	mapper["ad"]=["https://filters.adtidy.org/extension/chromium/filters/2.txt"];
	for(i = 0; i < extra_filters_check.length; i++) {
		ret=ret.concat(mapper[extra_filters_check[i]]);
	}
	return ret;
}
...
function checklastupdate(){
        chrome.storage.local.get(["lastupdate"], function(result) {
			...
			if(diff>86400000 && !refreshing_filters){
...
function refreshcounter(){
	chrome.storage.local.get("whitelist", function(result) {
		if(typeof result != 'undefined'){
			try{
			var res=JSON.parse(result["whitelist"]);
			}catch{var res={};}
		}else{var res={};}
		wt=res;
...
function checkall(req){
	...
	for (var j = 0; j < sublongest.length; j++) {	
		if(req["url"].toLowerCase().includes(sublongest[j].toLowerCase())){
			lu++;
...
function reqwhiteforce(ru,details){
	if(typeof details["url"]!="undefined"){
		...
		var rootdurl=rooturl(details["url"]);
		var exc=false;
		var comp="";
		if(ru in reqforce){
			var add=false;
			...
			for (var i = 0; i < ne; i++) {
				exc=false;
				comp=reqarr[i];
				if(reqarr[i].startsWith("@")){
					exc=true;
					comp=reqarr[i].replace("@","");
				}
				if(durl.includes(comp) && comp.length>2){
					if(exc){return "allow";}
					else{
				...
				if(comp=="*sc*" && details["type"]=="script" && rootdurl!=ru){
					return "block";
				}
				if(comp=="+"){
					add=true;
			...
			if(add){return "pass";}else{return "allow";}
		}
		for (var i = 0; i < reqforceinclude.length; i++) {
			if(ru.includes(reqforceinclude[i])){
				return reqwhiteforce([reqforceinclude[i]],details);
			}
		}
	}
	return "pass";
...
function isreqwhite(txt){
 ...
 chrome.webRequest.onBeforeRequest.addListener(
        function(details) {
			...
			if(isreqwhite(ru)){
				prov=reqwhiteforce(ru,details);
				if(prov=="block"){return {cancel: true};}
				if(prov=="allow"){return {cancel: false};}
				}
...
function insertautocookie(txt,tab){
	//THIS FUNCTION IS INACTIVE IN FIREFOX
	if(firefox){return;}
	var delay=200;
	if(txt.includes("||")){
		var arrt=txt.split("||");
...
function toskipcss(txt){
	...
	var carr;
	for (var i = 0; i < ne; i++) {
		if(skipcss[i].includes("|")){
			carr=skipcss[i].split("|");
			if(txt.includes(carr[0])&&txt.includes(carr[1])){return true;}
		}
		else{
			if(txt.includes(skipcss[i])){return true;}
		}
	}
	return false;
}
				...
				try {
					if(istopazf==false){
						...
						if(ru in cssrulewhite){
							...
							//console.log(modkeyopone+modall+sab_all_v.join(sepdisp)+sepdisp);
					...
					if(istopazs==true){
						...
						if(ru in cssrulewhite){
							...
							if(firefox){
								chrome.tabs.insertCSS(tab.id, {code: modkeyopthr+modall+sab_all_v.join(sepdisp)+sepdisp, allFrames:false, runAt:"document_start", cssOrigin : "user",matchAboutBlank : true},callback);
							}
							else{
							...
							if(firefox){
								chrome.tabs.insertCSS(tab.id, {code: modkeyopthr, allFrames:false, runAt:"document_start", cssOrigin : "user",matchAboutBlank : true},callback);
							}
							else
							{
...
function isexpected(url){
	...
	var url_slash=url;
	if(url.includes("/")){
		url_noslash = url.substring(url.indexOf("/") + 1, url.length);
		url_slash="/"+url_noslash;
	}
	var res=(exp==url || expp==url);
	res=res || (exp==url_slash || expp==url_slash);
	res=res || (exp==url_noslash || expp==url_noslash);
	return res;
}
	...
	chrome.tabs.get(lasttab,function callback() {if (chrome.runtime.lastError) { console.log(chrome.runtime.lastError.message);} else {
		chrome.tabs.get(lasttab,function(tabb) {
			...
			if(popupdanger(su) && !isabrowserpage(tab.url)){
				var s=isasearch(tab.url);
				if(!isexpected(tab.url) && !isnotdefinedyet(tab.url) && !s){
					tryremove(tab.id);
					notify(tabb.id,tab.url);
					return 0;
				}
