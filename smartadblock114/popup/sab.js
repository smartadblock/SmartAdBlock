...
function changeicon(ru,tabs){
	chrome.storage.local.get("whitelist", function(result) {
		...
		if(ru in res){
			document.getElementById("flag").setAttribute('src',simage);
			document.getElementById("dis").innerHTML = enable;
		}
		else{
			document.getElementById("flag").setAttribute('src','white.png');
...
function popinit(){
	...
	chrome.tabs.query({'active': true, 'lastFocusedWindow': true}, function (tabs) {
		...
		changeicon(ro,tabs);
		document.getElementById("site").innerHTML = ro.substring(0, 27);
